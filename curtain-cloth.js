...
const SEGX = 20;
const SEGY = 13;
const ITER = 5; // Увеличен релакс для предотвращения разрыва сетки
const DAMP = 0.91; // Усилено затухание для плавности
...
class Panel {
  ...
  build(aspect, texture) {
    ...
    // Стартуем сразу у бокового края, а не растянуто по всему экрану
    for (let y = 0; y <= SEGY; y++) {
      for (let x = 0; x <= SEGX; x++) {
        const wx = this.side < 0 ? -aspect : aspect;
  ...
  setPins(p) {
    const a = this.aspect;
    const gathered = a * 0.18; // Сбалансированная ширина сбора
    for (let x = 0; x <= SEGX; x++) {
      const t = x / SEGX;
      let closedX, openX;
      if (this.side < 0) {
        closedX = -a + t * a;
        openX = -a + t * gathered; // Убран резкий сдвиг за экран (-0.05)
      } else {
        closedX = t * a;
  ...
  step(mouse) {
    const pts = this.pts;
    const MAX_VEL = 0.035; // Ограничение максимальной скорости точки
...
class CurtainCloth extends HTMLElement {
  connectedCallback() {
    ...
    // WARMUP: Прогреваем физику на 35 кадров, чтобы ткань моментально приняла естественные складки
    for (let i = 0; i < 35; i++) {
